...
assert(Number(pkg.version.split('.')[1]) >= 18, 'package version must remain v18 or newer');

const today = read('src/components/today/TodayDashboard.tsx');
assert(today.includes("'Phase'"), 'Today must show the current phase');
assert(today.includes("'Ziel'"), 'Today must show the current goal');
assert(today.includes('HEUTE WICHTIG'), 'Today must show today’s task');
assert(today.includes('Dein nächster Schritt'), 'Today must expose a clear next step');
...
assert(nav.includes("id: 'today'") && nav.includes("id: 'smoking'") && nav.includes("id: 'pattern'") && nav.includes("id: 'progress'"), 'Navigation must preserve clear guided destinations');

const app = read('src/App.tsx');
assert(app.includes('global-want-to-smoke'), 'The acute craving action must be available app-wide');
assert(app.includes('SmokingDashboard'), 'The smoking quick-log must have a dedicated guided area');
