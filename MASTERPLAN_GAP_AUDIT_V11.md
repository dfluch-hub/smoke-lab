# SMOKE LAB — Masterplan Gap Audit (v11)
...
| Competing-task orchestration | Hardened in v11 | One-focus Today behavior plus consistent intervention priority across normal and parked-driving branches. |
...
| Protection plan priority | Hardened in v11 | Exact plan outranks Journey and moment-specific post-quit protection outranks generic Maintenance. |
...
| Structural local-state repair | Implemented in v11 | Duplicates, orphan links, inconsistent Journey state and stale experiment references are repaired deterministically at startup. |
| Critical profile validation | Implemented in v11 | Invalid profile payloads no longer enter the main UI as trusted state. |
...
5. remaining empty-state QA plus explicit forward migration/versioning behavior (structural corrupt/stale-state repair is now implemented in v11);
